...
def get_characters_and_times(lines_list:list, duration:int, multiline:bool=False, pause:int=1000):
    """
    ...
    if multiline:
            ...
            for i in range(len(characters)):
                if i == 0:
                    if len(total_characters) == 0:
                        ...
                        total_characters.append(total_characters[len(total_characters)-1] + characters[i])
                # once we reach the end of the line, add a line break
                elif i == len(characters)-1:
                    total_characters.append(total_characters[len(total_characters)-1] + line[i])
                    total_characters.append(total_characters[len(total_characters)-1] + "\n")
                else:
                    total_characters.append(total_characters[len(total_characters)-1] + line[i])
        ...
        for i in range(len(total_characters)):
            if i == 0:
                ...
                display_time.append(display_time[i - 1] + (duration) / len(total_characters) + (pause / 1000))
            # on the last character, add the pause time to the last keyframe
            elif i == len(total_characters)+1:
                display_time.append(display_time[i - 1] + (duration) / len(total_characters) + (pause / 1000))
            else:
                display_time.append(display_time[i - 1] + (duration) / len(total_characters))
